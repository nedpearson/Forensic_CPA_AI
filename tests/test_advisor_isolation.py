...
def setup_test_client():
    ...
    conn.execute("DELETE FROM company_memberships")
    ...
    cursor.execute("INSERT INTO company_memberships (company_id, user_id, role) VALUES (?, ?, ?)", (1, 1, "owner"))
    ...
    cursor.execute("INSERT INTO company_memberships (company_id, user_id, role) VALUES (?, ?, ?)", (2, 2, "owner"))
...
def test_authorized_company_access(setup_test_client):
    ...
    # Switch to the authorized company context
    client.post('/api/business/switch', json={'company_id': 1})
    
    # Call Advisor (company_id is pulled from session)
    res = client.get('/api/advisor/aggregate')
    assert res.status_code == 200
    data = res.get_json()
...
def test_cross_tenant_isolation(setup_test_client):
    ...
    # Attempt to switch context to Company 2 which User 1 doesn't own
    res = client.post('/api/business/switch', json={'company_id': 2})
    
    # Multi-tenant API MUST block this with a 403 Forbidden
    assert res.status_code == 403
    assert b"Unauthorized" in res.data or res.status_code == 403
